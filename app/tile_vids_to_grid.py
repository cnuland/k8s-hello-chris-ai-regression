...
def make_script(path):
    ...
    all_files = list(rollout_dir.glob("full_reset_0*.mp4"))
...
if __name__ == "__main__":
    ...
    if inner_mosaic:
        ...
        for script, sess in zip(scripts, all_sessions):
            out_file = Path(outer_dir / sess.with_suffix('.sh').name)
            write_file(out_file, script)
    else:
        base = Path('grid_renders')
        print(base.glob("session_*/full_reset_*.mp4"))
        all_input_vids = files = os.listdir(os.curdir)  
